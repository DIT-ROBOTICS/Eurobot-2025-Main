...
bool Recovery::setGoal(RosActionNode::Goal& goal) {
    ...
    auto m = getInput<geometry_msgs::msg::TwistStamped>("base");
    geometry_msgs::msg::TwistStamped robot_pose = getInput<geometry_msgs::msg::TwistStamped>("robot_pose").value();
    geometry_msgs::msg::TwistStamped rival_pose = getInput<geometry_msgs::msg::TwistStamped>("rival_pose").value();
    mission_type_ = getInput<int>("mission_type").value();
    ...
    // RCLCPP_INFO(logger(), "[Recovery]: Normal vector: " << normal_vector.twist.linear.x << " " << normal_vector.twist.linear.y << " " << normal_vector.twist.linear.z);
    // RCLCPP_INFO(logger(), "[Recovery]: Robot pose: " << robot_pose.twist.linear.x << " " << robot_pose.twist.linear.y);
    // RCLCPP_INFO(logger(), "[Recovery]: Rival pose: " << rival_pose.twist.linear.x << " " << rival_pose.twist.linear.y);
    // RCLCPP_INFO(logger(), "[Recovery]: Dot product: " << dot_product);
    cout << "[Recovery]: Normal vector: " << normal_vector.twist.linear.x << " " << normal_vector.twist.linear.y << " " << normal_vector.twist.linear.z << endl;
    cout << "[Recovery]: Robot pose: " << robot_pose.twist.linear.x << " " << robot_pose.twist.linear.y << endl;
    cout << "[Recovery]: Rival pose: " << rival_pose.twist.linear.x << " " << rival_pose.twist.linear.y << endl;
    cout << "[Recovery]: Dot product: " << dot_product << endl;
...
//     // if (kernel_ == nullptr) {
//     //     RCLCPP_ERROR(logger(), "Comparator: Kernel is null");
//     // }
...
//     geometry_msgs::msg::TwistStamped robot_pose;
//     geometry_msgs::msg::TwistStamped rival_pose;

//     // kernel_->GetRobotPosePtr(robot_pose);
//     // kernel_->GetRivalPosePtr(rival_pose);

//     RCLCPP_INFO(logger(), "Robot pose: " << robot_pose.twist.linear.x << " " << robot_pose.twist.linear.y);
//     RCLCPP_INFO(logger(), "Rival pose: " << rival_pose.twist.linear.x << " " << rival_pose.twist.linear.y);

//     if (Distance(pose, robot_pose) < Distance(pose, rival_pose)) {
//         // Log the distance
//         RCLCPP_INFO(logger(), "Distance: " << Distance(pose, robot_pose) << " " << Distance(pose, rival_pose));
...
// BT::NodeStatus BTMission::onStart() {
//     // if (kernel_ == nullptr) {
//     //     RCLCPP_ERROR(logger(), "[BTMission]: Kernel is null");
//     //     return BT::NodeStatus::FAILURE;
//     // }

//     auto m = getInput<std::deque<int>>("mission_type");
//     mission_type_ = m.value();

//     auto n = getInput<std::deque<int>>("mission_sub_type");
//     mission_sub_type_ = n.value();

//     // TODO: Register callback
//     // kernel_->RegisterMissionCallback(std::bind(&BTMission::MissionCompleteCB, this));

//     // RCLCPP_INFO(logger(), "[BTMission]: Send Mission type: " << mission_type_ << ", Sub type: " << mission_sub_type_);

//     // Send mission to kernel
//     if (mission_type_.size() > 0) {
//         SendMissionByQueue();
//     }
//     else {
...
// BT::NodeStatus BTMission::onRunning() {

//     bool need_mec_mission = (current_mission_sub_type_ == 6 || current_mission_sub_type_ == 8 || current_mission_sub_type_ == 10 || current_mission_sub_type_ == 12);

//     int current_mission_sub_type;
//     // kernel_->GetMecCallback(current_mission_sub_type);
//     if (send_mission == false && (current_mission_type_ == 0 || current_mission_sub_type == current_mission_sub_type_)) {
//         send_mission = true;
//         mission_type_.pop_front();
//         mission_sub_type_.pop_front();
//     }

//     if (!need_mec_mission || (need_mec_mission && !mec_callback_)) {
...
// void BTMission::SendMissionByQueue() {
//     if (send_mission == true) {
//         if (mission_type_.size() > 0) {
//             // kernel_->PubMissionFirmware(mission_type_.front(), mission_sub_type_.front());
//             RCLCPP_INFO(logger(), "[BTMission]: Send Mission type: " << mission_type_.front() << ", Sub type: " << mission_sub_type_.front());
//             current_mission_type_ = mission_type_.front();
//             current_mission_sub_type_ = mission_sub_type_.front();
//             // mission_type_.pop_front();
//             // mission_sub_type_.pop_front();
//             send_mission = false;
//         }
...
//     else {
//         // Resend the mission
//         // kernel_->PubMissionFirmware(current_mission_type_, current_mission_sub_type_);
//         RCLCPP_INFO(logger(), "[BTMission]: Send Mission type: " << mission_type_.front() << ", Sub type: " << mission_sub_type_.front());
//     }
// }

// void BTMission::MissionCompleteCB() {
//     if (mission_type_.size() > 0) {
//         SendMissionByQueue();
//     }
//     else {
