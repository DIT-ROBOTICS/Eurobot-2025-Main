...
bool NavigationTemp::setGoal(RosActionNode::Goal& goal) override
{
  getInput("order", goal.goal);
  RCLCPP_INFO(logger(), "Sending Goal");
  return true;
}

NodeStatus NavigationTemp::onResultReceived(const WrappedResult& wr) override
{
  RCLCPP_INFO(logger(), "Result received");
  return NodeStatus::SUCCESS;
}

NodeStatus NavigationTemp::onFailure(ActionNodeErrorCode error) override
{
