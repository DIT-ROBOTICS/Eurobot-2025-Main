...
int main(int argc, char** argv)
{
  ...
  auto nh = std::make_shared<rclcpp::Node>("bt_node");
  ...
  params.nh = nh;
  params.default_port_value = "navigationMain";
  factory.registerNodeType<Testing>("Testing");
  factory.registerNodeType<SimpleNavigation>("SimpleNavigation");
  factory.registerNodeType<TickFlow>("TickFlow");
  factory.registerNodeType<GeneratePathPoint>("GeneratePathPoint");
  factory.registerNodeType<BT::LoopNode<geometry_msgs::msg::TwistStamped>>
  ("LoopWayPoint");
  ...
  std::ofstream file("/home/user/groot_ws/groot_ws/src/bt_app/bt_config/bt_tree_node_model.xml");
  file << xml_tree_model;
  file.close();

  // Create the tree from the XML file
  std::string groot_xml_config_directory = "/home/user/groot_ws/groot_ws/src/bt_app/bt_config/";
